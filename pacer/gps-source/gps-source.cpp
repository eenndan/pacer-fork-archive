...
GPMFSource::~GPMFSource() noexcept {
  // Free the GPMF payload resource we own (allocated lazily, reused across
  // calls). Must happen before CloseSource. Previously every
  // ReadSamples()/ReadStream() call leaked a fresh resource.
  if (payload_res_) {
...
uint32_t GPMFSource::Seek(double target) {
  // `index_` is unsigned, so a `--index_` at index 0 wraps to UINT32_MAX (a
  // bogus, EOF-looking index). Initialise in/out so the post-loop guard never
  // reads garbage when the very first GetPayloadTime fails, and only step
  // `index_` back when the last lookup SUCCEEDED and we are not already at 0.
  // Net effect: a seek to before the first payload clamps to index 0 instead of
  ...
  do {
    ...
    if (ret == GPMF_OK) {
      if (out <= target)
        ++index_;
      // Step back toward the covering payload, but never below 0. At index 0 we
      // are already clamped, so stop the loop instead of spinning (the old code
      // let `--index_` wrap to UINT32_MAX here, after which GetPayloadTime
      // failed and broke the loop).
      if (target < in) {
        if (index_ == 0)
          break;
        --index_;
...
std::pair<double, double> GPMFSource::CurrentTimeSpan() const {
  double in = 0, out = 0;
  // No payload at index_ -> report an empty span rather than returning
  // uninitialised stack.
  if (GetPayloadTime(mp4handle_, index_, &in, &out) != GPMF_OK) {
    return {0, 0};
  }
...
// Callback contract identical to RawGPSSource::ReadSamples: (sample,
// current_index, total_records). Passed by reference into the per-codec parsers
// below.
using SampleEmit = std::function<void(GPSSample, uint32_t, uint32_t)>;

// Parse one GPS9 STRM payload (already positioned at samples by
// GPMF_SeekToSamples) and emit every fix. GPS9 element order: [lat, lon, alt,
// 2D speed, 3D speed, days since 2000, secs since midnight (ms precision), DOP,
// fix (0/2/3)]. Indices 3-6 (speeds + timestamp) are read unconditionally; a
// struct narrower than 7 elements would read past the row, so the caller guards
// `elements >= 7`. DOP(7)/fix(8) are separately guarded and otherwise keep the
...
void ParseGPS9(GPMF_stream *ms, uint32_t samples, uint32_t elements,
  ...
  // RAII scratch buffer (was a malloc/free pair). The malloc version leaked
  // whenever `emit` threw mid-loop (e.g. a Python callback raising through the
  // binding trampoline) — the vector unwinds cleanly. Same size, same layout,
  // same reads.
  ...
  for (uint32_t i = 0; i < samples; ++i) {
    ...
    // GPS9 quality: DOP (element 7) and fix type (element 8, 0/2/3). Present
    // only when the struct actually carries them; otherwise keep the sentinel
    // defaults.
    if (elements > 7) {
...
// Parse one GPS5 STRM payload (already positioned at samples) and emit every
// fix. `ms` is the GPS5 stream; `timestamp` is the (vestigial) GPSU time the
// caller resolved at the same level. GPS5 element order: [lat, lon, alt, 2D
// speed, 3D speed]; carries no DOP/fix, so those keep the GPSSample sentinels.
// Byte-identical to the former inline GPS5 branch.
void ParseGPS5(GPMF_stream *ms, uint32_t samples, uint32_t elements,
               int64_t timestamp, const SampleEmit &emit) {
  // GPS5 element order is [lat, lon, alt, 2D speed, 3D speed]. Skip a malformed
  // payload whose struct is too narrow to carry the five fields (guards the
  // explicit reads below).
  if (!(samples && elements >= 5)) {
    return;
  }
  // RAII scratch buffer (was a malloc/free pair) — see ParseGPS9; the malloc
  // version leaked when `emit` threw mid-loop.
  std::vector<double> tmpbuffer(static_cast<size_t>(samples) * elements);
  uint32_t buffersize =
      static_cast<uint32_t>(tmpbuffer.size() * sizeof(double));
  if (GPMF_OK != GPMF_ScaledData(ms, tmpbuffer.data(), buffersize, 0, samples,
  ...
  for (uint32_t i = 0; i < samples; ++i) {
    // Stride is the KNOWN element width (`elements`), not the UNITS-stream
    // repeat: when no SI_UNITS/UNITS sibling exists the old code's stride
    // collapsed to 1 and emitted 5 garbage samples per record. Write the fields
    // EXPLICITLY (same as the GPS9 branch) so 2D speed -> ground_speed and 3D
    // speed -> full_speed; the prior union field-order aliasing landed them
    // swapped (2D in full_speed) versus GPS9.
    GPSSample gps{};
    gps.lat = tmpbuffer[i * elements + 0];
    gps.lon = tmpbuffer[i * elements + 1];
    gps.altitude = tmpbuffer[i * elements + 2];
...
uint32_t GPMFSource::ReadSamples(
  ...
  // Reuse the owned payload resource (grows in place); freed once in the
  // destructor.
  ...
  if (payload == nullptr) {
    // No payload for this index (empty/EOF chunk). Return the error code
    // silently — the iteration protocol (is_end()/next()) already handles the
    // empty case, and this is on the chapter-seam hot path where a per-seam "No
    // payload" printf is pure console noise. GPMF_ERROR_MEMORY (== 1, "NULL
    // Pointer") is the parser's own code for GetPayload returning nullptr —
    // same value the old literal `return 1` produced.
  ...
  if (ret != GPMF_OK) {
    // Genuinely diagnostic (a corrupt/unparsable GPMF payload is NOT part of
    // normal iteration, unlike the empty-payload case above) — say what failed
    // and where, on stderr so it never pollutes piped stdout.
  ...
  while (GPMF_OK ==
    ...
    if (ret != GPMF_OK) {
      // A sample-less STRM is unremarkable (the caller just sees the nonzero
      // code); the old per-occurrence "No Seek to Samples" printf carried zero
      // information and could spam the console once per payload on sources that
      // hit it. Dropped, behavior unchanged.
      return ret;
    }
    ...
    if (key == STR2FOURCC("GPS5")) {
      ...
      if (GPMF_OK ==
        ...
        // TODO: GPS5 per-sample timing is VESTIGIAL — this loop keeps only the
        // LAST GPSU timestamp and stamps every sample in the payload with it.
        // GPS9 (the live path) carries a true per-sample fix time; GPS5 timing
        // is not reworked in this batch.
...
// Reads one GPMF stream (e.g. ACCL/GRAV/CORI) over EVERY payload of this MP4,
// emitting each sample with a media-clock timestamp. The timing mirrors the
// research dump_imu.c: a payload covers the media span [in,out] (from
// GetPayloadTime, the same clock as the GPS payload spans), and the `samples`
// rows in it are spread evenly as in + (out-in)*(s/samples). This keeps the IMU
// on the identical media clock the GPS / video sync uses. nelem is clamped to 4
...
void GPMFSource::ReadStream(
  ...
  // Reuse the owned payload resource (shared with ReadSamples(); grows in
  // place); freed in dtor.
  for (uint32_t i = 0; i < payloads; ++i) {
    ...
    if (GPMF_OK ==
      ...
      // RAII scratch buffer (was a malloc/free pair) — see ParseGPS9; the
      // malloc version leaked when `emit` threw mid-loop.
      std::vector<double> tmpbuffer(static_cast<size_t>(samples) * elements);
      uint32_t buffersize =
          static_cast<uint32_t>(tmpbuffer.size() * sizeof(double));
...
// Multi-chapter: read each child's stream, shifting later chapters by the
// cumulative duration of the earlier ones (the same global media clock the GPS
// spans use). The shared per-stream logic lives in the ReadShifted template;
// these three just bind the matching reader verb.
...
uint32_t SequentialGPSSource::Seek(double target) {
  if (auto left_duration = left_->GetTotalDuration(); target < left_duration) {
    ...
    // left_->GetTotalDuration(); for a nested SequentialGPSSource that
    // recomputation walked the whole left subtree again. Same value, one
    // traversal.
...
void SequentialGPSSource::Next() {
  // At the left->right chapter seam, position `right_` AT its first payload and
  // return without advancing. The old code switched to `right_` (fresh at index
  // 0) and then immediately called Next() (++index_), silently dropping the
  // first payload of every chapter after the first.
...
// Base default: a source with no GPS payload emits nothing. Concrete sources
// (GPMFSource / SequentialGPSSource — or a Python subclass via the binding
// trampoline) override.
